...
uint8_t MAX30102_WriteReg(uint8_t RegAddr, uint8_t Data)
{
    ...
    /*发送写入地址*/
    MyI2C_SendByte(MAX30102_WR_ADDR);
    if(MyI2C_WaitAck() != 0)
    {
        MyI2C_Stop();
        return 0;
...
uint8_t MAX30102_ReadReg(uint8_t RegAddr)
{
    ...
    /*发送读取地址*/
    MyI2C_SendByte(MAX30102_RD_ADDR);
    if(MyI2C_WaitAck() != 0)
    {
        MyI2C_Stop();
        return 0;
    ...
    MyI2C_SendAck(DISACK);
    MyI2C_Stop();
    return data;
}

/**
 * @brief   从MAX30102读取FIFO数据
 * @param   *buf: 指向存储读取数据的指针
 * @param   len: 要读取的数据长度
 * @retval  无
 */
void MAX30102_ReadFIFO(uint8_t *buf, uint8_t len)
{
	MAX30102_ReadReg(MAX30102_INT_STATUS1);
	MAX30102_ReadReg(MAX30102_INT_STATUS2);
	
    MyI2C_Start();
    MyI2C_SendByte(MAX30102_WR_ADDR);
    MyI2C_WaitAck();
    MyI2C_SendByte(MAX30102_FIFO_DATA);   // 0x07
    MyI2C_WaitAck();
    MyI2C_Start();                         // 重复起始
    MyI2C_SendByte(MAX30102_RD_ADDR);
    MyI2C_WaitAck();
...
void MAX30102_Init(void)
{
    ...
    GPIO_Init_Structure.GPIO_Pin = MAX30102_INT_Pin;
    GPIO_Init_Structure.GPIO_Mode = GPIO_Mode_IPU;   //上拉输入
    GPIO_Init(MAX30102_INT_Port, &GPIO_Init_Structure);
	
	MAX30102_WriteReg(MAX30102_MODE_CONFIG, 0x40);	//重置模块
	MAX30102_WriteReg(MAX30102_MODE_CONFIG, 0x40);
	
    /*模块启动*/
    /*配置中断使能寄存器*/
    MAX30102_WriteReg(MAX30102_INT_ENABLE1, 0xC0);   //只使能FIFO数据满和新数据就绪中断
    MAX30102_WriteReg(MAX30102_INT_ENABLE2, 0x00);    //禁用温度就绪中断

    /*配置FIFO寄存器*/
    MAX30102_WriteReg(MAX30102_FIFO_WR_PTR, 0x00);  //重置写指针
	MAX30102_WriteReg(MAX30102_FIFO_OVF_CNT, 0x00);  //重置溢出计数器
	MAX30102_WriteReg(MAX30102_FIFO_RD_PTR, 0x00);  //重置读指针

    /*配置FIFO参数*/
    /*样本平均数=8，禁用溢出回滚，FIFO几乎满=17*/
    MAX30102_WriteReg(MAX30102_FIFO_CONFIG, 0x0F);  //011 0 'F
    ...
    /*配置SpO2参数*/
    /*ADC量程=8192nA，SpO2采样率=100sps，LED脉冲宽度=411us*/
    MAX30102_WriteReg(MAX30102_SPO2_CONFIG, 0x27);   //0 10 0'01 11

    /*配置LED驱动电流*/
    MAX30102_WriteReg(MAX30102_LED1_PA, 0x24);
    MAX30102_WriteReg(MAX30102_LED2_PA, 0x24);
	MAX30102_WriteReg(MAX30102_PILOT_PA, 0x7F);
}
